...
const MainPage = (): React.ReactElement => {
    ...
    useEffect(() => {
        getServer();
        const pingServer = setInterval(getServer, 30000)
        return () => clearInterval(pingServer)
    }, [store])

    useEffect(() => {
        ipcRenderer.send('launcher:get')
        ipcRenderer.send('launcher:getStore')
        const handleLauncherStatus = (_event, data): void => {
            if (data) {
                setStatus('Играть');
            } else {
        ...
        ipcRenderer.on('launcher:get', handleLauncherStatus);
    ...
    const handleUsername = (e: ChangeEvent<HTMLInputElement>): void => {
        ...
        ipcRenderer.send('launcher:setNickname', e.target.value)
    }
